    ...
    if createnewtl:
        ...
        async def pick(statusLabel):
            try:
                ...
                btn.destroy()
                command = f'"{path.join(getcwd()[:2],"/Program Files/","7-Zip/","7z.exe")}" x {EXE_PATH.name} -o{path.join(path.dirname(EXE_PATH.name),"OSO_NVIDIA_DEBLOAT")} -y'
                ...
                EXTRACTED_DIR = path.join(path.dirname(EXE_PATH.name),"OSO_NVIDIA_DEBLOAT")
                ...
                with open(path.join(EXTRACTED_DIR,"NVI2","presentations.cfg"),'w', encoding='utf-8') as f:
                    f.writelines(filtered)
                statusLabel.configure(text="Debloat completed successfully!\nFeel free to remove the OSO_NVIDIA_DEBLOAT folder next to where you installed your driver to.")
