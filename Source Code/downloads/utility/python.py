...
async def getURL(ssl_ctx,continuation,progressbar,completeDownload):
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://www.python.org/downloads/",ssl=ssl_ctx) as resp:
        ...
        return
    url = re.findall(r"https:\/\/www\.python\.org\/ftp\/python\/.{,20}\/python-.{,20}-amd64\.exe",html)[0]
    print(url)
